...
int gauss_eliminate_using_openmp(Matrix, Matrix);
...
int 
 main(int argc, char** argv) {
     ...
     * */
	gettimeofday(&start, NULL);
	status = gauss_eliminate_using_openmp(A, U);
	gettimeofday(&stop, NULL); 
...
int gauss_eliminate_using_openmp(Matrix A, Matrix U)                  /* Write code to perform gaussian elimination using OpenMP. */
{ 
	unsigned int i, j, k;
	int n = A.num_rows; 
	float * fuck; 
